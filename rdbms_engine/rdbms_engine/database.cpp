...
table Database::naturalJoin(table table_one, table table_two){
	...
	table final_table(table_name, new_table_column_names, new_table_column_types, new_table_primary_keys);
	return final_table;
}
...
table Database::setUnion(table table_one, table table_two)
{
	...
	for(unsigned int i = 0; i<two_num_entities; ++i)
	{
